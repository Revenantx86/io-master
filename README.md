...
Use the following `make` targets to generate layouts.

### IHP SG13G2 Build
Generates the padring and merges the final GDS for the IHP node.
...
These variables define the environment and tool paths.

| Variable | Description |
| :--- | :--- |
| **`PDK_ROOT`** | Root directory containing the Process Design Kits (PDKs). |
| **`PADRING_BIN`** | Path to the `padring` executable (generates IO ring). |
| **`KLAYOUT_BIN`** | Path to the `klayout` executable (layout viewer/editor). |
| **`IHP_IO_LEF`** | Path to the IHP SG13G2 IO Library Exchange Format (LEF) file. |
| **`GF180_IO_LEF`** | Path to the GF180MCU IO Library Exchange Format (LEF) file. |
